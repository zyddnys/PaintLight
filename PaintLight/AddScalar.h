...
public:
	AddScalar() : m_cs(nullptr), m_buf(nullptr)
	{
		;
	}
	AddScalar(ID3D11Device *device, ID3D11DeviceContext *context)
	{
		ID3DBlob *csByteCodes{ nullptr };
		THROW(CompileShader(L"AddScalar.hlsl", nullptr, "main", "cs_5_0", std::addressof(csByteCodes)));
	...
	AddScalar(AddScalar const &other) = delete;
	AddScalar &operator=(AddScalar const &other) = delete;

	AddScalar(AddScalar &&other) noexcept : m_cs(other.m_cs), m_buf(other.m_buf)
	{
		other.m_cs = nullptr;
		other.m_buf = nullptr;
	}
	AddScalar &operator=(AddScalar &&other) noexcept
	{
